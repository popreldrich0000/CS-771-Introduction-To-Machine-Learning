...
def get_index(self,complete_words,my_words_indices,history):
	q_idx = -1
	maxInformationGain = 0
	for idx in my_words_indices:
		cnt_set_sizes = {}
		for j in my_words_indices:
			cover = self.reveal(complete_words[j],complete_words[idx])
			if cover not in cnt_set_sizes:
				cnt_set_sizes[cover] = 0
			cnt_set_sizes[cover]+=1
		curInfoGain = np.log2(len(my_words_indices))
		for (cover,cnt) in cnt_set_sizes.items():
			p = cnt/len(my_words_indices)
			curInfoGain -= p*np.log2(cnt)
		if curInfoGain > maxInformationGain:
			maxInformationGain = curInfoGain
			q_idx = idx
	return q_idx
class Tree:
...
class Node:
	...
	def process_node( self, all_words, my_words_idx, history, verbose ):
		...
		if len( history ) == 0:
			...
			query_idx = self.get_index(all_words,my_words_idx,history)
